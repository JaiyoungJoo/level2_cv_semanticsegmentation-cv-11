...
def make_dataset(debug="False"):
    # dataset load
    tf = A.Resize(512, 512)
    train_dataset = XRayDataset_Multi(is_train=True, transforms=tf)
    valid_dataset = XRayDataset_Multi(is_train=False, transforms=tf)
    if debug=="True":
...
def validation(epoch, model, data_loader, criterion, thr=0.5):
    ...
    with torch.no_grad():
        ...
        for step, (images, masks, ages, genders, weights, hights) in tqdm(enumerate(data_loader), total=len(data_loader)):
            images, masks, ages, genders, weights, hights = images.cuda(), masks.cuda(), ages.cuda(), genders.cuda(), weights.cuda(), hights.cuda()     
            model = model.cuda()

            outputs, ages_, genders_, weights_, hights_ = model(images, ages, genders, weights, hights)
...
def train(model, data_loader, val_loader, criterion, optimizer, args):
    ...
    for epoch in range(args.epochs):
        ...
        for step, (images, masks, ages, genders, weights, hights) in enumerate(data_loader):            
