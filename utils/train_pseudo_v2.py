...
# dataset
import dataset
from dataset import XRayDataset, XRayInferenceDataset_pseudo
...
tf = A.Resize(1024, 1024)
test_dataset = dataset.XRayInferenceDataset_pseudo(transforms=tf, pseudo = True)
...
def save_model(model, args):
    ...
    output_path = '/opt/ml/input/weights/HRNet_final/2_Pretrained_smp_HRNet_comb_loss_tf=True_cln=True_e=100_sd=up.pt'
...
# def pseudo_infer(model, data_loader, thr=0.5,tta_enabled=False):
...
def train(model, data_loader, val_loader, test_loader, criterion, optimizer, args):
    ...
    # 여기부터 슈도 들어감
        ...
        # model = torch.load('/opt/ml/input/weights/HRNet_final/2_Pretrained_smp_HRNet_comb_loss_tf=True_cln=True_e=100_sd=up.pt')
        model_infer = torch.load('/opt/ml/input/weights/HRNet_final/2_Pretrained_smp_HRNet_comb_loss_tf=True_cln=True_e=100_sd=up.pt')
        model = model.cuda()
        ...
        #     pseudo_images.append(test_images)
        #     pseudo_outputs.append(test_outputs)
        ...
        for step, ((images, masks), (test_images, image_name)) in tqdm(enumerate(zip(data_loader, test_loader)), total=len(data_loader)):   
            with torch.no_grad():       
                ...
                # tta_model = tta.SegmentationTTAWrapper(model_infer, tta_transforms)
                # pseudo_masks = tta_model(test_images)
                pseudo_masks = model_infer(test_images)
            # gpu 연산을 위해 device 할당
            images, masks = images.cuda(), masks.cuda()
            # print('####합치기 전')
            # print('images_outputs.shape', images.shape)
            ...
            outputs2 = model(test_images)
            # loss 계산
            loss = criterion(outputs1, masks) + criterion(outputs2, pseudo_masks)*1/2
            # loss = criterion(outputs1, masks)#criterion(outputs2, pseudo_masks)
        ...
        if (epoch + 1) % args.val_every == 0:
            ...
            if best_dice < dice:
                ...
                best_dice = dice
                pseudo_weight_path = '/opt/ml/level2_cv_semanticsegmentation-cv-11/pseudo/best_{epoch+1}.pt'
...
if __name__ == '__main__':
    ...
    args.save_dir = '/opt/ml/input/weights/pseudo/'
